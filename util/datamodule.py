...
class MultiVariateDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(multi_vars.size(0)):
            multi_vars[i] = self.normalize(multi_vars[i])
        return \
...
class MultiSalesDataset(Dataset):
    ...
    def __preprocess__(self):
        ...
        self.multi_vars = torch.FloatTensor(self.data_dict['multi_vars'].values)
        # self.multi_vars[:27] = self.normalize(self.multi_vars[:27])
        # self.multi_vars[27:34] = self.normalize(self.multi_vars[27:34])
        # self.multi_vars[34:50] = self.normalize(self.multi_vars[34:50])
        # self.multi_vars[50:52] = self.normalize(self.multi_vars[50:52])
        
        # for i in range(self.multi_vars.size(0)):
        #     self.multi_vars[i] = self.normalize(self.multi_vars[i])
